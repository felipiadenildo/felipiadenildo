# Felipi Adenildo Soares Sousa

[English](#english) · [Português](#português) · [中文](#中文)

---

## English

Computer Engineer graduated from the University of São Paulo (USP), with professional experience in software development and cloud automation, and applied research in artificial intelligence and computer vision.

**Education:** Bachelor's in Computer Engineering — São Carlos School of Engineering (EESC) and Institute of Mathematical and Computer Sciences (ICMC), University of São Paulo.

**Experience:** Professional experience in software development and cloud process automation in the financial sector, and in technology consulting. Undergraduate thesis in computer vision applied to wildlife monitoring.

**Areas of interest:** artificial intelligence and computer vision, cloud automation, embedded systems.

**Languages:** Portuguese (native), English (fluent), Spanish (advanced), Mandarin and Japanese (basic).

**Contact:** [LinkedIn](https://www.linkedin.com/in/felipiadenildo/) · [personal website](https://felipiadenildo.github.io) · felipiadenildo@gmail.com

---

## Português

Engenheiro de Computação formado pela Universidade de São Paulo (USP), com experiência profissional em desenvolvimento de software e automação em nuvem, e pesquisa aplicada em inteligência artificial e visão computacional.

**Formação:** Bacharelado em Engenharia de Computação — Escola de Engenharia de São Carlos (EESC) e Instituto de Ciências Matemáticas e de Computação (ICMC), Universidade de São Paulo.

**Experiência:** Atuação profissional em desenvolvimento de software e automação de processos em nuvem no setor financeiro, e em consultoria de tecnologia. Pesquisa de conclusão de curso em visão computacional aplicada ao monitoramento de fauna.

**Áreas de interesse:** inteligência artificial e visão computacional, automação em nuvem, sistemas embarcados.

**Idiomas:** português (nativo), inglês (fluente), espanhol (avançado), mandarim e japonês (básico).

**Contato:** [LinkedIn](https://www.linkedin.com/in/felipiadenildo/) · [site pessoal](https://felipiadenildo.github.io) · felipiadenildo@gmail.com

---

## 中文

计算机工程师，毕业于圣保罗大学（USP），拥有软件开发与云自动化方面的专业经验，并从事人工智能与计算机视觉的应用研究。

**学历：** 计算机工程学士 —— 圣保罗大学圣卡洛斯工程学院（EESC）与数学计算科学研究所（ICMC）。

**工作经历：** 曾在金融行业从事软件开发与云端流程自动化的专业工作，亦有技术咨询经验。本科毕业论文研究计算机视觉在野生动物监测中的应用。

**兴趣领域：** 人工智能与计算机视觉、云自动化、嵌入式系统。

**语言能力：** 葡萄牙语（母语）、英语（流利）、西班牙语（高级）、中文与日语（基础）。

**联系方式：** [LinkedIn](https://www.linkedin.com/in/felipiadenildo/) · [个人网站](https://felipiadenildo.github.io) · felipiadenildo@gmail.com
